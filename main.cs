  ...
  public static void Main (string[] args) {
    ...
    while (continuar == "s") {
      ...
      continuar = Console.ReadLine();
		...
		//itens e total carrinho
		double totalCarrinho=0;
		Console.WriteLine("\n\n*****Seu Carrinho*****");
		for (int i=0; i<novoCarrinho.prodsCarrinho.Count; i++){
			Console.WriteLine("Prod {0} - {1} - Qtd.: {2} - R${3} ", i+1, novoCarrinho.prodsCarrinho[i].getProduto(), novoCarrinho.prodsCarrinho[i].getEstoque(), novoCarrinho.prodsCarrinho[i].getPreco());
    ...
    while (!aprovado){
      ...
      if (totalCarrinho <= cartaoUm.getLimite()){
        ...
        //recusado
        Console.WriteLine("\n\nCompra recusada por falta de Limite! Por favor, cadastre outro cartão.\n\n");
      }
    }
  }
}